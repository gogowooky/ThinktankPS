...
#region static TTAppManager
#########################################################################################################################
class TTAppManager {

    static [System.Windows.Window]$_window

    static [void] Initialize(){
        [xml]$xaml = Get-Content ( $script:TTScriptDirPath + "\thinktank.xaml" )
        [TTAppManager]::_window = [System.Windows.Markup.XamlReader]::Load( (New-Object XmlNodeReader $xaml) )
        [TTAppManager]::_window.Add_Loaded( $script:Window_Loaded )
        [TTAppManager]::_window.Add_PreviewKeyDown( $script:Window_PreviewKeyDown )
        [TTAppManager]::_window.Add_PreviewKeyUp( $script:Window_PreviewKeyUp )

    }
    static [void] Window( [string]$state ){ # max / min / normal / close 
        switch( $state ){
            'max'    { [TTAppManager]::_window.WindowState = [System.Windows.WindowState]::Maximized }
            'min'    { [TTAppManager]::_window.WindowState = [System.Windows.WindowState]::Minimized }
            'normal' { [TTAppManager]::_window.WindowState = [System.Windows.WindowState]::Normal }
            'close'  { [TTAppManager]::_window.Close() }
        }
    }
    static [void] Border( [string]$id, [int]$percent ){ 

        switch( $id ){
            'Layout.Guide.Width' {
                $script:ThinktankGrid.ColumnDefinitions[0].Width = "$percent*"
                $script:ThinktankGrid.ColumnDefinitions[1].Width = "$(100-$percent)*"
            }
            'Layout.Library.Height' {
                $script:GuideGrid.RowDefinitions[0].Height = "$percent*"
                $script:GuideGrid.RowDefinitions[1].Height = "$(100-$percent)*"
            }
            'Layout.Shelf.Height' {
                $script:WorkplaceGrid.RowDefinitions[0].Height = "$percent*"
                $script:WorkplaceGrid.RowDefinitions[1].Height = "$(100-$percent)*"
            }
            'Layout.Work1.Width' {
                $script:LRDivdDeskGrid.ColumnDefinitions[0].Width = "$percent*"
                $script:LRDivdDeskGrid.ColumnDefinitions[1].Width = "$(100-$percent)*"
            }
            'Layout.Work1.Height' {
                $script:ULDivdDeskGrid.RowDefinitions[0].Height = "$percent*"
                $script:ULDivdDeskGrid.RowDefinitions[1].Height = "$(100-$percent)*"
            }
        }
        $script:app._set( $id, [string]$percent )
    }

    static [object] FindName( [string]$name ){
        return [TTAppManager]::_window.FindName( $name )
    }
    static [void] Show(){
        [TTAppManager]::_window.ShowDialog()
    }
...
#region static TTLibraryManagerManager
#########################################################################################################################
# class TTAppManager {
