...
class TTApplicationController {
    ...
    TTApplicationController(){
        ...
        $this.status = $global:ResMan.GetChild( "Status" )
        $this.configs = $global:ResMan.GetChild( "Configs" )
            ...
            "(?<name>Editor|Browser|Grid)(?<num>[123])" {
                ...
                switch($name){ # Desk caption
                    'Editor' {
                        ...
                        $title = $global:ResMan.GetChild('Memos').GetChild($index).Title
...
class TTGroupController {
    ...
    [TTGroupController] load( [string]$panel, [string]$name ){
        ...
        $res = $global:ResMan.GetChild( $name )
...
class TTEditorController {
    ...
    [TTEditorController] create( [int]$no ){
        $index = $global:ResMan.GetChild('Memos').CreateChild()
    ...
    [void] _paste_image( $type, $editor, $text ){ # photoタグとの運用を考える
        ...
        $folder = $global:ResMan.GetChild('Configs').GetChild("CaptureFolder").value
    ...
    [void] on_save( $params ){
        ...
        #### Memos
        $memo = $global:ResMan.GetChild('Memos').GetChild($index)
        ...
        $global:ResMan.GetChild('Editings').AddChild( $editor, $foldings )
    ...
    [void] on_load( $params ){
        ...
        #### Editings
        $editing = $global:ResMan.GetChild('Editings').GetChild($index)
