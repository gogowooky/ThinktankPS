...
class TTApplication {   
    ...
    [void] border( $name, $value ){
        ...
        [TTAppManager]::Border( $name, $percent )
...
class TTListPanel {
    ...
    [TTListPanel] initialize( $collection ){ # $this._name を設定してから呼び出すこと
        ...
        $this._keyword  = $script:AppMan.FindName("$($this._name)")
...
function Show_ListMenu( $collection ){
    ...
    $script:ListMenuWindow = $null
    $script:ListMenu = $null
    $script:ListMenuWindow = [Markup.XamlReader]::Load( (New-Object XmlNodeReader $script:ListMenuXaml) )
...
[ScriptBlock] $script:ListMenu_Loaded = {
    ...
    # if( 0 -eq $script:ListMenuLeft ){
        [System.Windows.Point] $pt = $script:WorkplaceGrid.PointToScreen([System.Windows.Point]::New(0, 0))
...
class TTPopupMenu{
    ...
    TTPopupMenu( $items ){
        $this._items = $script:PopupMenuWindow.FindName("PopupMenuItems")
        $this._caption = $script:PopupMenuWindow.FindName("PopupMenuCaption")

        $style = [Style]::new()
        $style.Setters.Add( [Setter]::new( [Controls.GridViewColumnHeader]::VisibilityProperty, [Visibility]::Collapsed ) )
        $this._items.view.ColumnHeaderContainerStyle = $style
    ...
    [TTPopupMenu]items( $items ){
        ...
        $width = (
            $this._items.ItemsSource.foreach{
                [System.Text.Encoding]::GetEncoding("shift_jis").GetByteCount( $_ )
            } | Measure-Object -Max
        ).Maximum

        $this._items.view.Columns[0].Width = $width * 10
        $this._items.Width = $width * 10
        $this._items.Height = ($this._items.Items.Count + 1) * 22.5 + 15  

        $this._items.SelectedItem = $this._items.ItemsSource.where{ $_ -like "@*" }[0]

        return $this
    }
    [int] height(){
        return $this._items.Height
    }
    [int] width(){
        return $this._items.Width
    }
    [TTPopupMenu]reload( $items ){
        $index = $this._items.SelectedIndex
        $this.items( $items )
        $this._items.SelectedIndex = $index
        return $this
    }
    [string]selected(){
        return $this._items.SelectedItems
    }
    [TTPopupMenu]refresh(){
        $this._items.Items.refresh()
        return $this
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
    ...
    xmlns:d="http://schemas.microsoft.com/expression/blend/2008"
    xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006"
    Title="ListWindow_mini" Name="PopupMenu" WindowStyle="None" AllowsTransparency="True">
...
#region　actions and events
#------------------------------------------------------------------------------------------------------------------------
function ShowPopupMenu( $items, $modkey, $key, $title, $editor ){

    [TTSusModMode]::Set( $title, $modkey, $key )


    $script:PopupMenuWindow = [Markup.XamlReader]::Load( (New-Object XmlNodeReader $script:PopupMenuXaml) )
    $script:PopupMenu = [TTPopupMenu]::New( $items )
    $script:PopupMenu.caption( $title )

    $script:PopupMenuWindow.Add_Deactivated( $PopupMenu_Deactivated )
    $script:PopupMenuWindow.Add_MouseDoubleClick( $PopupMenu_MouseDoubleClick )
    $script:PopupMenuWindow.Add_PreviewKeyUp( $PopupMenu_PreviewKeyUp )
    $script:PopupMenuWindow.Add_PreviewKeyDown( $PopupMenu_PreviewKeyDown )

    $top = $editor.TextArea.TextView.VisualLines[0].VisualTop
    $pos = $editor.TextArea.TextView.GetVisualPosition( $editor.TextArea.Caret.Position, [Rendering.VisualYPosition]::BaseLine )
    $pos.y = $pos.y - $top 
    [System.Windows.Point]$point = $editor.TranslatePoint( $pos, $null )
    $script:PopupMenuWindow.Top = $point.Y + $script:AppMan.Position.Top + 20
    $script:PopupMenuWindow.Left = $point.X + $script:AppMan.Position.Left + 30
    $script:PopupMenuWindow.Width  = $script:PopupMenu.width()
    $script:PopupMenuWindow.Height = $script:PopupMenu.height()

    if( $false -eq $script:PopupMenuWindow.ShowDialog() ){
        return $null
    }else{
        return $script:PopupMenu._items.SelectedItem
    }
}

[ScriptBlock] $PopupMenu_Deactivated = {
    if( $script:PopupMenuWindow.Visibility -eq [System.Windows.Visibility]::Visible ){ 
        [TTSusModMode]::Clear()
        $script:PopupMenuWindow.Hide()
    }
}
[ScriptBlock] $PopupMenu_MouseDoubleClick = {
    $script:PopupMenuWindow.DialogResult = $True 
    $script:PopupMenuWindow.Hide()
    $args[1].Handled = $True
}
[ScriptBlock] $PopupMenu_PreviewKeyUp = {
    if( [TTSusModMode]::Check( $args[1].Key ) ){
        $script:PopupMenuWindow.DialogResult = $True
        $script:PopupMenuWindow.Hide()
        $args[1].Handled = $True
    }
}
[ScriptBlock] $PopupMenu_PreviewKeyDown = {
    $key  = $args[1].Key
    $mod  = $args[1].KeyboardDevice.Modifiers
    $skey = $args[1].SystemKey
    ...
    switch( $key ){
        'Escape' { $window.DialogResult = $False; $window.Close(); $args[1].Handled = $True; return }
    }

    :Handled switch( [TTSusModMode]::Name ){
        "Date" {
            if( [TTSusModMode]::Mod -eq 'Alt'){ $key = $skey }  
            switch( $mod ){
                "$([TTSusModMode]::Mod)" {
                    switch( $key ){
                        "$([TTSusModMode]::Key)" { $popup.cursor('down+'); break Handled }
                        ...
                        'Q'     { $window.DialogResult = $False; $window.Close(); break Handled }
                        'O'     { $popup.reload($dt.AddFormat(1).tags()); break Handled }
                        'Right' { $popup.reload($dt.AddFormat(1).tags()); break Handled }
                        'Left'  { $popup.reload($dt.AddFormat(-1).tags()); break Handled }
                        'Y'     { $popup.reload($dt.AddYears(1).tags()); break Handled }
                        'M'     { $popup.reload($dt.AddMonths(1).tags()); break Handled }
                        'D'     { $popup.reload($dt.AddDays(1).tags()); break Handled }
                        'W'     { $popup.reload($dt.AddDays(7).tags()); break Handled }
                        'C'     { $popup.reload($dt.date((Get-Date)).tags()); break Handled }
                        'R'     { $popup.reload($dt.restore().tags()); break Handled }
                        default { return }
                    }
                }
                "$([TTSusModMode]::Mod), Shift" {
                    switch( $key ){
                        "$([TTSusModMode]::Key)" { $popup.cursor('up+'); break Handled }
                        'O'     { $popup.reload($dt.AddFormat(-1).tags()); break Handled }
                        'Y'     { $popup.reload($dt.AddYears(-1).tags()); break Handled }
                        'M'     { $popup.reload($dt.AddMonths(-1).tags()); break Handled }
                        'D'     { $popup.reload($dt.AddDays(-1).tags()); break Handled }
            ...
            switch( $mod ){
                "$([TTSusModMode]::Mod)" {
                    switch( $key ){
                        ...
                        'Down'  { $popup.cursor('down'); break Handled }
                        'Q'     { 
                            $script:PopupMenuWindow.DialogResult = $False
                            $script:PopupMenuWindow.Close()
                            break Handled
                        }
                        default { return }
                    }
